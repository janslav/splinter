...
{
    interface IAssemblyCode
    {
        MethodDefinition GetMethodByFullName(string fullName);
    }

    /// <summary>
    /// Class representing the code of a .NET assembly.
    /// Originally this was the Module class from NinjaTurtles
    /// </summary>
    public class AssemblyCode : Splinter.Phase2_Mutation.IAssemblyCode
    {
        ...
        public MethodDefinition GetMethodByFullName(string fullName)
        {
            return this.methodsByFullName.GetOrAdd(
                fullName,
                n => this.AssemblyDefinition.Modules.SelectMany(m => m.Types).SelectMany(t => t.Methods).Single(m => m.FullName.Equals(n)));
        }
